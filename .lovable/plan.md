...
1. Pre-publish readiness checks
   - Run a fresh security scan. The most recent scan is from 2026-08-01 and is marked stale; publishing is blocked by unresolved critical findings.
   - Verify the public policy pages (Terms, Refund Policy, Privacy) are accessible at the root routes `/terms`, `/refund-policy`, `/privacy` and include the correct seller identity (Jeff Hallstead).
